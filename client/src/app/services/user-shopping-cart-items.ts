...
export class UserShoppingCartItemsService {
  ...
  public itemToAdd(itemToAdd: ShoppingCartItemModel): Promise<ShoppingCartItemModel> {
    return this.http.post<ShoppingCartItemModel>(apiBaseURL + "/cart/actions", itemToAdd, { headers: authHeaders.createHeader(store.getState().token) }).toPromise();
  }
  ...
  public async removeItemFromCart(productId: number): Promise<boolean> {
    try {
      ...
      const index = cartItems.findIndex(c => c.id == productId);
      // get cart total price from redux store
      let cartTotalPrice = +store.getState().cartTotalPrice;
  ...
  public itemToRemove(productId: number): Promise<ShoppingCartItemModel[]> {
    return this.http.delete<ShoppingCartItemModel[]>(apiBaseURL + "/cart/actions/" + productId , { headers: authHeaders.createHeader(store.getState().token) }).toPromise();
  }
